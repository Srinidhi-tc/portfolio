...
function TennisBallSVG({ size = 26 }) {
  const id = "tb";
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none"
      ...
      <ellipse cx="16" cy="29" rx="10" ry="2.5" fill="#3A6010" opacity="0.18" />
      <circle cx="16" cy="15.5" r="13.5"
              fill={`url(#${id}-body)`} filter={`url(#${id}-shadow)`} />
      <g clipPath={`url(#${id}-clip)`}>
        <path d="M 4 22 C 7 18, 10 10, 18 8 C 22 7, 26 8, 28 10"
      ...
      <circle cx="16" cy="15.5" r="13.5"
              fill={`url(#${id}-glint)`} clipPath={`url(#${id}-clip)`} />
    </svg>
  );
}
...
export default function TennisBall() {
  ...
  const launch = useCallback(() => {
    ...
    const toY   = dogRect.top  + 16;

    const fly = flyRef.current;

    // Pre-position at nav ball before showing
    ...
    // Phase 1 — fly TO dog head
    animateArc({
      flyEl, fromX, fromY, toX, toY,
      cpY:       outboundCpY,
      duration:  750,
      rotations: 2.5,
      easing:    easeInOut,
    ...
    <>
      <button
        ...
        style={{
          ...
          transform:      hovered ? "scale(1.18) rotate(10deg)" : "scale(1) rotate(0deg)",
